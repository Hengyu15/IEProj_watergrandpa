...
var GameScene = cc.Scene.extend({
    ...
    initData: function () {
        ...
        //添加背景
        var bg = cc.Sprite.create(s_dryland);
...
//          var moveby = cc.MoveBy(1,cc.p(0,10));
//          this.flowerList[i].File.runAction(moveby);
        }
    },
    //初始化叶子
    initLeaf: function () {
    ...
    initWaterdrop: function(){
    	var left = 0;
    	var space = 30;
    	for (var i = 1; i <= 15; i++) {
            //添加15个
            var prize = new WaterdropPrize();
            prize.initData();
            prize.setPosition(cc.p(left + i * space, 270));
            this.gameLayer.addChild(prize, g_GameZOder.ui);
            prize.setVisible(false);
            this.waterdropList.push(prize);
        }
    },
...
//  }
    drop: function (i) {
        //for(var j = 0;  j< 5; j++){
            setTimeout(this.waterdropList[i].y-=20,1000);
            //setInterval(this.waterdropList[i].y-=20,1000*(j+1));
            //setTimeout(move(this.waterdropList[i].y),1000);
            //}
    },
...
//                  bigImg.src = "res/owl1.png";
        ...
        for (var i = 0; i < this.acornList.length; i++) {
            ...
            if (!prize.isHit) {
                if (this.bear.collide(prize)) {
                    prize.setVisible(false);   //隐藏
                    this.waterdropList[i].setVisible(true);
                    //this.waterdropList[i].setPosition(cc.pAdd(this.getPosition(), cc.pAdd((0,20), dt)));
                    //this.flowerList[i].File.runAction(moveby);
                    //this.waterdropList[i].timer(); 
                    //this.waterdropList[i].y=200;
                    //this.waterdropList[i].move();
                    for(var x=0; x<5;x++){
                    	this.drop(i);
                    }
