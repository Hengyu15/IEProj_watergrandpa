    ...
    for(let i=0;i<li.length;i++){
      li[i].addEventListener('click', function() {
        if(_this.timeflag === false){
          ...
          _this.timeflag = true;
          if(outputdiv){
            let outhtml = '<div class="outdiv outstep"><span class="outstep_star">Game start!</span><span class="outdiv_span outstep_span"></span></div>';
//          outhtml += '<div class="outdiv outpro">游戏进度：<span class="outdiv_span outpro_span">0/'+total+'</span></div>';
        ...
        if(data1==null && data2==null){
          ...
          _this.step++; //计数器
          if(outputdiv){
            let stephtml = _this.step+' time(s)';
            if(_this.step<=1){
              outputdiv.getElementsByClassName('outstep_star')[0].innerHTML = 'You have tried ';
            }
