...
class WPDA_List_Table_Menu extends WPDA_List_Table
{
    ...
    public function single_row( $item )
    {
        ...
        if ( $this->bulk_actions_enabled ) {
            echo  '<tr style="height:0 !important; padding: 0 !important;"><td id="rownum_' . esc_attr( self::$list_number ) . '_x1" colspan="' . esc_attr( 13 - $column_not_showns ) . '" style="padding:0 !important;"></tr>' ;
            // Fake! Maintain odd/even colors.
            echo  '<tr style="padding: 0 !important;"><td style="padding: 0 !important;"></td><td id="rownum_' . esc_attr( self::$list_number ) . '_x2" colspan="' . esc_attr( 12 - $column_not_showns ) . '" style="padding:0 10px 10px 0 !important;">' ;
        } else {
            echo  '<tr style="height:0 !important; padding: 0 !important;"><td id="rownum_' . esc_attr( self::$list_number ) . '_x1" colspan="' . esc_attr( 12 - $column_not_showns ) . '" style="padding:0 !important;"></tr>' ;
            // Fake! Maintain odd/even colors.
            echo  '<tr style="padding: 0 !important;"><td id="rownum_' . esc_attr( self::$list_number ) . '_x2" colspan="' . esc_attr( 12 - $column_not_showns ) . '" style="padding:0 10px 10px 10px !important;">' ;
        }
    ...
    public function column_default( $item, $column_name )
    {
        ...
        if ( 'table_rows' === $column_name ) {
            ...
            if ( 'federated' === strtolower( $item['engine'] ) || 'connect' === strtolower( $item['engine'] ) || 'innodb' === strtolower( $item['engine'] ) || stripos( strtolower( $item['table_type'] ), 'view' ) !== false ) {
        ...
        if ( 'table_name' === $column_name ) {
            ...
            if ( WPDA::can_manage() ) {
                ...
                $actions['manage'] = "<a href=\"javascript:void( 0 )\" class='wpda_tooltip' title='{$table_actions_title}' onclick=\"wpda_show_table_actions( '{$this->schema_name}', '{$table_name}', '" . self::$list_number++ . "', '{$wp_nonce_table_actions}', '{$item['table_type_db']}', '" . self::LOADING . "' ); this.blur();\">" . '<span class="material-icons wpda_icon_on_button" style="width:20px">apps</span>' . __( 'Manage', 'wp-data-access' ) . '</a>';
            }
            ...
            $action_view = '<span class="material-icons wpda_icon_on_button" style="width:20px">view_list</span>' . __( 'Explore', 'wp-data-access' );
    ...
    protected function process_bulk_action_edit_db()
    {
        ...
        // input var okay.
        $passwd = ( isset( $_REQUEST['edit_remote_passwd'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['edit_remote_passwd'] ) ) : '' );
        // input var okay.
    ...
    protected function process_bulk_action_create_db()
    {
        ...
        if ( isset( $_REQUEST['database_location'] ) && 'local' === $_REQUEST['database_location'] ) {
            ...
            // input var okay.
            $passwd = ( isset( $_REQUEST['remote_passwd'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['remote_passwd'] ) ) : '' );
            // input var okay.
    ...
    protected function process_bulk_action_delete_menu()
    {
        ...
        if ( isset( $_REQUEST['menu_table_name'] ) ) {
            $menu_table_name = sanitize_text_field( wp_unslash( $_REQUEST['menu_table_name'] ) );
            // input var okay.
        ...
        if ( isset( $_REQUEST['menu_id'] ) && isset( $_REQUEST['menu_id'] ) ) {
            $menu_id = sanitize_text_field( wp_unslash( $_REQUEST['menu_id'] ) );
            // input var okay.
    ...
    protected function process_bulk_action_settings_table()
    {
        ...
        if ( isset( $_REQUEST['settings_table_name'] ) && isset( $_REQUEST['settings'] ) ) {
            $settings_table_name = sanitize_text_field( wp_unslash( $_REQUEST['settings_table_name'] ) );
            // input var okay.
            $settings_object = json_decode( WPDA::sanitize_text_field_array( $_REQUEST['settings'] ) );
            ...
            if ( 'dashboard_menus' === $settings_object->request_type ) {
                ...
                if ( isset( $settings_object->table_settings->hyperlink_definition ) && isset( $settings_object->unused ) ) {
                    // Save table settings
            ...
            } elseif ( 'column_settings' === $settings_object->request_type ) {
                ...
                if ( $this->process_bulk_action_check_wpnonce( "wpda-copy-{$copy_table_name_src}", '_wpnonce' ) ) {
                    ...
                    if ( false === $dbo_type || 'SYSTEM VIEW' === $dbo_type ) {
                        ...
                        if ( false === $this->copy_table( $copy_table_name_src, $copy_table_name_dst, $include_data ) ) {
                            ...
                            $msg = new WPDA_Message_Box( array(
                                'message_text' => strtoupper( substr( $dbo_type, 0, 1 ) ) . strtolower( substr( $dbo_type, 1 ) ) . ' `' . $copy_table_name_src . '` ' . __( 'copied to', 'wp-data-access' ) . ' `' . $copy_table_name_dst . '` ',
                            ) );
                            $msg->box();
                        }
    ...
    protected function copy_table( $copy_table_name_src, $copy_table_name_dst, $include_data )
    {
        ...
        if ( 'on' === $include_data ) {
            $result = $wpdadb->query( "create table `{$copy_table_name_dst}` like `{$copy_table_name_src}`" );
            // db call ok; no-cache ok.
            
            if ( false === $result ) {
                return false;
            } else {
                return $wpdadb->query( "insert `{$copy_table_name_dst}` select * from `{$copy_table_name_src}`" );
                // db call ok; no-cache ok.
            }
        
        } else {
            return $wpdadb->query( "create table `{$copy_table_name_dst}` like `{$copy_table_name_src}`" );
            // db call ok; no-cache ok.
        }
    ...
    protected function add_header_button( $add_param = '' )
    {
        ...
        ?>"
			...
			>
				<div>
					...
					>
						<span class="material-icons wpda_icon_on_button">add_circle</span> <?php 
        ...
        ?>"
			...
			><span class="material-icons wpda_icon_on_button">cloud_download</span> <?php 
        echo  __( 'Data Backup' ) ;
        ?></a>
			<?php 
    }
    ...
    protected function add_db_containers()
    {
        ...
        ?>">
							...
							<input type="text" name="edit_remote_passwd" id="edit_remote_passwd" value="<?php 
        ...
        ?>">
						...
						<input type="submit" class="button button-prrimary" value="<?php 
        echo  __( 'Save', 'wp-data-access' ) ;
        ?>">
						<a href="javascript:void(0)"
						   onclick="jQuery('#wpda_db_edit').hide()"
						   class="button button-secondary"><span class="material-icons wpda_icon_on_button">cancel</span> <?php 
        ...
        ?>>
						...
						<a href="javascript:void(0)"
						   ...
						   class="button button-primary"><span class="material-icons wpda_icon_on_button">save</span> <?php 
        echo  __( 'Save', 'wp-data-access' ) ;
        ?></a>
						<a href="javascript:void(0)"
